...
# Cos
# max threads: ???
function SCMC_cos_kernel(OUT::String, v1::String, varlist::Vector{String}, sparsity::Vector{Int})
    if startswith(v1, "temp")
        v1_cv = "$(v1)_cv"
        v1_cc = "$(v1)_cc"
        v1_lo = "$(v1)_lo"
...
function SCMC_quadaff_initialize(CONST::Real)
    ...
    # Reset subgradients to 0, since they only get added to in quadaff expressions
    write(buffer, "        ##################################\n")
